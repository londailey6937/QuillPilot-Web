...
const POSITIVE_WORDS = new Set([
  "happy", "joy", "smile", "laugh", "love", "delight", "pleased", "wonderful",
  "excellent", "perfect", "beautiful", "amazing", "brilliant", "fantastic",
  "hope", "excited", "thrilled", "grateful", "proud", "confident", "warm",
  "gentle", "kind", "caring", "peaceful", "calm", "relieved", "content"
]);

const NEGATIVE_WORDS = new Set([
  "sad", "angry", "fear", "hate", "terrible", "awful", "horrible", "bad",
  "worse", "worst", "cruel", "harsh", "pain", "hurt", "wound", "blood",
  "death", "die", "kill", "scream", "cry", "tears", "grief", "despair",
  "anxious", "worried", "scared", "frightened", "terrified", "nervous",
  "bitter", "resentful", "jealous", "guilty", "ashamed", "disgusted"
]);
...
function extractCharacterMentions(
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    const end = Math.min(text.length, position + characterName.length + contextRadius);
...
function calculateEmotionalTrajectory(mentions: CharacterMention[]): {
  ...
  const avgSentiment = (mentions: CharacterMention[]) => {
    ...
    const sum = mentions.reduce((acc, m) => acc + sentimentToScore(m.sentiment), 0);
 ...
 */
function determineArcType(trajectory: { early: number; middle: number; late: number }): "dynamic" | "flat" | "unclear" {
 ...
 */
function determineRole(mentions: number, totalMentions: number): "protagonist" | "major" | "supporting" | "minor" {
...
export function analyzeCharacters(text: string): CharacterAnalysisResult {
  ...
  if (characterNames.size === 0) {
    return {
      ...
      recommendations: ["No characters detected. Ensure character names are capitalized and mentioned multiple times."],
    };
  }
  
  // Calculate total mentions across all characters
  const totalMentions = Array.from(characterNames.values()).reduce((sum, count) => sum + count, 0);
  ...
  for (const [name, count] of characterNames.entries()) {
    ...
    const developmentScore = calculateDevelopmentScore(arcType, trajectory, count);
    
    const dialogueMentions = mentions.filter(m => m.inDialogue).length;
    const dialogueRatio = mentions.length > 0 ? dialogueMentions / mentions.length : 0;
  ...
  const avgDevelopment = characters.length > 0
    ? Math.round(characters.reduce((sum, c) => sum + c.developmentScore, 0) / characters.length)
    : 0;
  ...
  if (protagonists.length === 0) {
    recommendations.push("No clear protagonist detected. Consider giving your main character more presence.");
  }
  
  const flatProtagonists = characters.filter(c => c.role === "protagonist" && c.arcType === "flat");
  if (flatProtagonists.length > 0) {
    recommendations.push(`${flatProtagonists[0].name} shows little emotional change. Consider adding character development.`);
  }
  
  const minorCharacterCount = characters.filter(c => c.role === "minor").length;
  if (minorCharacterCount > characters.length * 0.7) {
    recommendations.push("Many minor characters detected. Consider consolidating or giving some more depth.");
  }
  
  if (avgDevelopment < 50) {
    recommendations.push("Character development scores are low. Add more emotional depth and transformation.");
  }
