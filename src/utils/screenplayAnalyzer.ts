 ...
 */
function extractScreenplayBlocks(html: string): Array<{type: string, text: string, lineNumber: number}> {
 ...
 * Parse scene heading
 */
function parseSceneHeading(heading: string): { type: string, location: string, timeOfDay: string } {
  ...
  if (upperHeading.startsWith('INT.') || upperHeading.startsWith('INT ')) {
    type = 'INT';
  } else if (upperHeading.startsWith('EXT.') || upperHeading.startsWith('EXT ')) {
    type = 'EXT';
  } else if (upperHeading.startsWith('INT/EXT') || upperHeading.startsWith('INT./EXT')) {
    type = 'INT/EXT';
  } else if (upperHeading.startsWith('EST.') || upperHeading.startsWith('EST ')) {
    type = 'EST';
  }
  ...
  if (type !== 'OTHER') {
    ...
    const timeWords = ['DAY', 'NIGHT', 'MORNING', 'EVENING', 'DUSK', 'DAWN', 'CONTINUOUS', 'LATER', 'SAME'];
 ...
 */
export function analyzeScreenplay(html: string, plainText: string): ScreenplayMetrics {
  ...
  blocks.forEach((block, index) => {
    switch (block.type) {
      ...
      case 'character':
        ...
        char.dialogueCount++;
        if (currentSceneIndex >= 0 && !char.scenes.includes(currentSceneIndex)) {
  ...
  const dialogueToActionRatio = totalActionBlocks > 0 ? totalDialogueBlocks / totalActionBlocks : 0;
  const averageDialogueLength = totalDialogueBlocks > 0 ? totalDialogueWords / totalDialogueBlocks : 0;
  const averageActionBlockLength = actionBlockLengths.length > 0
    ? actionBlockLengths.reduce((a, b) => a + b, 0) / actionBlockLengths.length
    : 0;
  ...
  if (estimatedPages < 90) {
    formattingIssues.push(`Script is short (${estimatedPages} pages). Industry standard is 90-120 pages.`);
  } else if (estimatedPages > 120) {
    formattingIssues.push(`Script is long (${estimatedPages} pages). Industry standard is 90-120 pages.`);
  }
  
  if (totalScenes < 30) {
    formattingIssues.push(`Low scene count (${totalScenes}). Most feature scripts have 40-60 scenes.`);
  } else if (totalScenes > 80) {
    formattingIssues.push(`High scene count (${totalScenes}). Scenes may be too short. Consider combining.`);
  }
  
  if (dialogueToActionRatio > 2) {
    formattingIssues.push('Dialogue-heavy script. Consider adding more visual action.');
  } else if (dialogueToActionRatio < 0.5) {
    formattingIssues.push('Action-heavy script. May need more dialogue for character development.');
  }
  
  if (longActionBlocks > totalActionBlocks * 0.2) {
    formattingIssues.push(`${longActionBlocks} action blocks exceed 4 lines. Break into smaller paragraphs for readability.`);
  }
  
  // Check character balance
  const sortedChars = Array.from(characters.values()).sort((a, b) => b.dialogueWordCount - a.dialogueWordCount);
  if (sortedChars.length > 1 && sortedChars[0].dialogueWordCount > totalDialogueWords * 0.4) {
    formattingIssues.push(`Character "${sortedChars[0].name}" dominates dialogue (${Math.round(sortedChars[0].dialogueWordCount / totalDialogueWords * 100)}%). Consider balancing.`);
  }
...
export function generateScreenplaySummary(metrics: ScreenplayMetrics): string {
  ...
  lines.push(`- **Average Scene Length:** ${metrics.averageSceneLength} blocks`);
  lines.push(`- **Act 1:** Pages 1-${metrics.act1EndPage}`);
  lines.push(`- **Act 2:** Pages ${metrics.act1EndPage + 1}-${metrics.act2EndPage}`);
  lines.push(`- **Act 3:** Pages ${metrics.act2EndPage + 1}-${metrics.act3EndPage}\n`);
  
  // Location & Time
  lines.push('## Settings');
  lines.push(`- **Interior Scenes:** ${metrics.interiorScenes} (${Math.round(metrics.interiorScenes / metrics.totalScenes * 100)}%)`);
  lines.push(`- **Exterior Scenes:** ${metrics.exteriorScenes} (${Math.round(metrics.exteriorScenes / metrics.totalScenes * 100)}%)`);
  ...
  lines.push(`- **Average Dialogue Length:** ${metrics.averageDialogueLength} words`);
  ...
  if (topCharacters.length > 0) {
    ...
    topCharacters.forEach((char, i) => {
      const percentage = Math.round((char.dialogueWordCount / metrics.totalDialogueBlocks) * 100);
      lines.push(`${i + 1}. ${char.name}: ${char.dialogueCount} dialogue blocks, ${char.dialogueWordCount} words (${percentage}%)`);
    });
  }
  lines.push('');
  ...
  lines.push(`- **Average Action Block Length:** ${metrics.averageActionBlockLength} lines`);
  lines.push(`- **Long Action Blocks (>4 lines):** ${metrics.longActionBlocks}\n`);
