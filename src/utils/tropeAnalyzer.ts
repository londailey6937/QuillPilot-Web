  ...
  storyBeats: [
    {
      ...
      keywords: ["meet", "first", "encounter", "bump", "unexpected"],
    },
    {
      name: "Initial Attraction",
      position: "early" as const,
      keywords: ["notice", "drawn", "attracted", "beautiful", "handsome"],
    },
    {
      name: "Conflict/Obstacle",
    ...
    },
    {
      name: "Dark Moment",
      position: "middle" as const,
      keywords: ["apart", "break", "leave", "ending", "over", "goodbye"],
    },
    {
      name: "Grand Gesture",
  ...
  storyBeats: [
    {
      ...
      position: "early" as const,
      keywords: ["attack", "murder", "threat", "danger", "crime", "incident"],
    },
    {
      name: "Call to Action",
      position: "early" as const,
      keywords: ["investigate", "pursue", "hunt", "find", "stop", "prevent"],
    },
    {
      name: "False Victory",
      position: "middle" as const,
      keywords: ["solved", "caught", "safe", "over", "relief", "celebrate"],
    },
    {
      name: "Major Setback",
      position: "middle" as const,
      keywords: ["worse", "wrong", "trap", "ambush", "lose", "fail"],
    },
    {
      name: "Final Confrontation",
      position: "late" as const,
      keywords: ["face", "confront", "showdown", "final", "end this", "battle"],
    },
    {
      name: "Resolution",
      position: "late" as const,
      keywords: ["justice", "caught", "safe", "peace", "truth", "exposed"],
    },
  ],
};
  ...
  storyBeats: [
    {
      name: "Ordinary World",
      position: "early" as const,
      keywords: ["normal", "simple", "peaceful", "village", "home", "routine"],
    },
    {
      name: "Call to Adventure",
      position: "early" as const,
      keywords: ["summon", "call", "need", "must", "leave", "journey"],
    },
    {
      name: "Crossing Threshold",
      position: "early" as const,
      keywords: ["leave", "depart", "beyond", "enter", "cross", "new world"],
    },
    {
      name: "Trials and Tests",
      position: "middle" as const,
      keywords: ["challenge", "test", "trial", "prove", "overcome", "struggle"],
    },
    {
      name: "Approach the Cave",
      position: "middle" as const,
      keywords: [
        "stronghold",
        "fortress",
    ...
    },
    {
      name: "Ordeal",
      position: "late" as const,
      keywords: ["battle", "fight", "confront", "face", "defeat", "victory"],
    },
    {
      name: "Return with Prize",
      position: "late" as const,
      keywords: ["return", "home", "changed", "victory", "peace", "restored"],
    },
  ],
};
  ...
  storyBeats: [
    {
      ...
      position: "early" as const,
      keywords: ["found", "body", "murder", "crime", "dead", "discovered"],
    },
    {
    ...
    {
      name: "Gathering Suspects",
      position: "middle" as const,
      keywords: ["suspect", "interview", "alibi", "motive", "opportunity"],
    },
    {
      name: "False Solution",
      position: "middle" as const,
      keywords: ["arrest", "solved", "guilty", "confession", "caught"],
    },
    {
      name: "True Revelation",
      position: "late" as const,
      keywords: ["actually", "real", "truth", "discover", "reveal", "expose"],
    },
    {
      name: "Denouement",
      position: "late" as const,
      keywords: ["explain", "how", "why", "confession", "evidence", "proof"],
    },
  ],
};
...
const SCIFI_TROPES = {
  ...
  storyBeats: [
    ...
    },
    {
      ...
      position: "middle" as const,
      keywords: ["should", "ethics", "right", "wrong", "consequence", "moral"],
    },
    {
      name: "System Failure",
      position: "middle" as const,
      keywords: ["malfunction", "fail", "error", "corrupt", "breach", "crisis"],
    },
    {
      name: "Paradigm Shift",
      position: "late" as const,
      keywords: ["change", "revolution", "transform", "new era", "evolution"],
    },
    {
  ...
  storyBeats: [
    {
      name: "Ominous Setup",
      position: "early" as const,
      keywords: ["wrong", "strange", "unsettling", "eerie", "feeling", "sense"],
    },
    {
      name: "First Encounter",
      position: "early" as const,
      keywords: ["see", "glimpse", "hear", "sound", "shadow", "movement"],
    },
    {
      name: "Escalating Terror",
      position: "middle" as const,
      keywords: ["worse", "more", "again", "closer", "intense", "fear"],
    },
    {
      name: "False Safety",
      position: "middle" as const,
      keywords: ["safe", "relief", "escape", "away", "over"],
    },
    {
      name: "Final Horror",
      position: "late" as const,
      keywords: ["face", "confront", "final", "truth", "reveal", "ultimate"],
    },
  ],
};
...
export function analyzeTropes(
  ...
  // Detect tropes
  const detectedTropes: Trope[] = library.tropes
    .map((trope) => {
      let matches = 0;
  ...
  const subversionScore = calculateSubversionScore(
    detectedTropes,
    storyBeats,
    library
  );
 ...
 */
function divideIntoSections(text: string): {
  early: string;
  middle: string;
  late: string;
} {
 ...
 */
function calculateSubversionScore(
  tropes: Trope[],
  beats: StoryBeat[],
  library: typeof ROMANCE_TROPES
): number {
  ...
  const tropeAbsenceScore =
    ((expectedTropeCount - detectedTropeCount) / expectedTropeCount) * 100;

  const expectedBeatCount = library.storyBeats.length;
  const detectedBeatCount = beats.filter((b) => b.detected).length;
  const beatAbsenceScore =
    ((expectedBeatCount - detectedBeatCount) / expectedBeatCount) * 100;
...
function generateTropeRecommendations(
  ...
  if (detectedBeats.length > 0) {
    recommendations.push(
      `Story beats found: ${detectedBeats
        .map((b) => b.name)
        .join(", ")}. These provide good narrative structure.`
    );
  }

  if (missingBeats.length > 0 && missingBeats.length <= 3) {
    recommendations.push(
      `Consider adding: ${missingBeats
        .slice(0, 2)
        .map((b) => b.name)
        .join(", ")}. These beats can strengthen your ${genre} narrative arc.`
    );
  }
